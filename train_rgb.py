...
def train():
    with tf.Graph().as_default():
        ...
        var_keep_dic = tf.train.list_variables(os.path.join(BASE_DIR, 'log'))
        var_ckpt = [var_shape[0] for var_shape in var_keep_dic]    
        variables = tf.global_variables()
        variables_to_restore = get_variables_to_restore(variables, var_ckpt)
